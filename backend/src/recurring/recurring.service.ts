...
import { equalSplit, exactSplit, percentageSplit, ShareResult, SplitInput } from '../expenses/split-calculator';
...
export class RecurringService {
  ...
  private async validateAndBuildSplitConfig(
    groupId: string,
    dto: Pick<CreateRecurringExpenseDto, 'paidById' | 'splitType' | 'participantUserIds' | 'splits'>,
  ): Promise<SplitConfig> {
    ...
    if (!memberIds.has(dto.paidById)) throw new BadRequestException('paidById must be a member of this group');

    if (dto.splitType === SplitType.EQUAL) {
      const participantUserIds = dto.participantUserIds ?? [];
      for (const id of participantUserIds) {
        if (!memberIds.has(id)) throw new BadRequestException(`User ${id} is not a member of this group`);
      }
      if (participantUserIds.length === 0) throw new BadRequestException('At least one participant is required');
      return { participantUserIds };
    }

    const splits = dto.splits ?? [];
    for (const s of splits) {
      if (!memberIds.has(s.userId)) throw new BadRequestException(`User ${s.userId} is not a member of this group`);
    }
    if (splits.length === 0) throw new BadRequestException('At least one split is required');
    return { splits };
  }

  private computeShares(amountCents: number, splitType: SplitType, config: SplitConfig): ShareResult[] {
    if (splitType === SplitType.EQUAL) return equalSplit(amountCents, config.participantUserIds ?? []);
    if (splitType === SplitType.EXACT) return exactSplit(amountCents, config.splits ?? []);
    return percentageSplit(amountCents, config.splits ?? []);
  }

  async create(requesterId: string, groupId: string, dto: CreateRecurringExpenseDto) {
  ...
  async listForGroup(requesterId: string, groupId: string) {
    await this.groupsService.assertMember(groupId, requesterId);
    return this.prisma.recurringExpense.findMany({ where: { groupId }, orderBy: { createdAt: 'desc' } });
  }

  async setActive(requesterId: string, id: string, active: boolean) {
    const recurring = await this.prisma.recurringExpense.findUnique({ where: { id } });
    if (!recurring) throw new NotFoundException('Recurring expense not found');

    const membership = await this.groupsService.assertMember(recurring.groupId, requesterId);
    if (recurring.createdById !== requesterId && membership.role !== GroupRole.OWNER) {
      throw new ForbiddenException('Only the creator or the group owner can modify this recurring expense');
    }
  ...
  private async runOnce(recurring: {
    ...
    const shares = this.computeShares(recurring.amountCents, recurring.splitType, config);
  ...
  async processDueRecurringExpenses(): Promise<void> {
    ...
    for (const recurring of due) {
      ...
      for (let i = 0; i < MAX_CATCH_UP_RUNS && current.nextRunAt <= new Date(); i++) {
        await this.runOnce(current);
        const refreshed = await this.prisma.recurringExpense.findUnique({ where: { id: current.id } });
